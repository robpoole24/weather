...
async function pollAlerts() {
  ...
  try {
    ...
    for (const feature of features) {
      ...
      const event = restoreGeographicNames(props.event || 'Advisory');
      ...
      const headline = restoreGeographicNames(props.headline || event);
      const areaDesc = restoreGeographicNames((props.areaDesc || '').split(';')[0].trim());
